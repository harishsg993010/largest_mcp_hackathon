...
/**
 * Type for the custom tool registry
 */
...
/**
 * Error thrown when a tool is not in the registry
 */
export class ToolNotInRegistryError extends Error {
  constructor(toolName: string) {
    super(`Tool "${toolName}" is not in the allowed registry`);
    this.name = 'ToolNotInRegistryError';
  }
}
 ...
 */
function checkToolInRegistry(toolName: string, registry?: MCPToolRegistry): boolean {
  if (!registry || !registry.enforceRegistry) {
    return true;
  }
  return registry.allowedTools.includes(toolName);
}
...
export class MCPClient {
  ...
  async callTool(toolName: string, input?: unknown) {
    // Check if tool is in registry before proceeding
    if (!checkToolInRegistry(toolName, { allowedTools: ['get_stock_price', 'buy_stock'], enforceRegistry: true })) {
      throw new ToolNotInRegistryError(toolName);
    }
